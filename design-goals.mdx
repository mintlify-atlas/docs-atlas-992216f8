...
rules_xcodeproj supports one more build mode called "Build with Proxy". In this mode the generated project relies on Xcode using an [XCBBuildServiceProxy](https://github.com/MobileNativeFoundation/XCBBuildServiceProxyKit). This takes the Xcode build system entirely out of the equation, allowing Bazel to fully control the build.

Benefits the proxy provides over "Build with Bazel":
...
Despite these benefits, "Build with Bazel" remains the default mode for two reasons:
...
For teams where the benefits outweigh these inconveniences, "Build with Proxy" remains a supported option, with "Build with Bazel" always available as a fallback.
